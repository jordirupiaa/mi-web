...
/**
 * `services.name` / `services.description` live in Supabase — currently in
 * English, since the admin hasn't updated them to Spanish yet — and there's
 * no per-language column (this project deliberately avoids adding one
 * without a real migration decision). Rather than assume the DB text is
 * already in a particular language, all six known real rooms are translated
 * here for every language this site offers, matched by name (accepting both
 * the original English names and the Spanish ones they may be renamed to)
 * so it keeps working no matter what's currently in the database. Any room
 * that doesn't match one of these — a brand-new room type the admin adds
 * later — falls back to the raw text from the database in every language,
 * which is always correct, just not translated yet.
 */
...
type TranslatedLanguage = 'es' | 'en' | 'fr' | 'ca' | 'de' | 'it'

const ROOM_TRANSLATIONS: Array<{ pattern: RegExp; text: Record<TranslatedLanguage, RoomText> }> = [
  {
    pattern: /single|individual/i,
    text: {
  ...
  {
    pattern: /twin.*window|twin.*ventana/i,
    text: {
  ...
  {
    pattern: /twin.*balcony|twin.*balc[oó]n/i,
    text: {
  ...
  {
    pattern: /quadruple|cu[aá]druple/i,
    text: {
...
export function translateRoom(service: Pick<ServiceRow, 'name' | 'description'>, language: string): RoomText {
  const fallback: RoomText = { name: service.name, description: service.description ?? '' }

  const match = ROOM_TRANSLATIONS.find((r) => r.pattern.test(service.name))
  if (!match) return fallback

  const translated = match.text[language as TranslatedLanguage]
